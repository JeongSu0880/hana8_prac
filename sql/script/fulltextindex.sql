...
-- explain으로 어떤 인덱스를 사용했는지 확인하기
-- type에 aLL 나오면 그냥 다 뒤졌다는 뜻임.

select * from Notice where match(title, contents) against('조선');
explain select * from Notice where match(title, contents) against('조선');
-- 왜 row 1? 찾는거 index DB에서 다 찾고 한방에 Notice DB에서 가져왔다는 뜻.
-- 아 근데 왜? 왜 결과가 다름? 알려줘..

-- 검색 엔진. 다음은 키워드 하나가 돈이 될거라고 생각을 못 했지. 요즘에는 키워드 사고 팜 
-- 부산대에서 만든 바른 한글 
...
-- 이제 이렇게 하면 '조선의'도 나온다.!!
-- like의 '%'와는 다르게 *조선* 일때 맨 앞의 것ㅁㄴㅇ럼ㄴ어ㅗㄻㄴㅇㄹ (고조선이 안 나온다는 뜻)

select * from Notice where match(title, contents) against(? IN boolean mode);
 -- 프로젝트에서는 이렇게 활용해라. 
